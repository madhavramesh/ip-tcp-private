...
char* chooseString(const struct config *conf, enum branchType type, int life, int dx, int dy) {
	...
	branchStr = (char *)malloc(maxStrLen);
	...
	while (life > 0) {
		...
		/* 		) ) { */
			...
			// otherwise create a shoot
				...
				// create shoot
				branch(conf, objects, myCounters, y, x, (enum branchType)((myCounters->shootCounter % 2) + 1), shootLife);
			}
		}
		shootCooldown--;
		if (conf->verbosity > 0) {
...
char* createDefaultCachePath(void) {
	...
	if (env_XDG_CACHE_HOME && (envlen = strlen(env_XDG_CACHE_HOME))) {
		...
		result = (char *)malloc(envlen + strlen(toAppend) + 1);
	...
	toAppend = "cbonsai";
	result = (char *)malloc(strlen(toAppend) + 1);
